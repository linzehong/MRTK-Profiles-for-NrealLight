    ...
    public class LinePointer : BaseControllerPointer
    {
        ...
        protected virtual void PreUpdateLineRenderers()
        {
            using (PreUpdateLineRenderersPerfMarker.Auto())
            {
                ...
                Debug.Log($"lincon PreUpdateLineRenderers IsFocusLocked:{IsFocusLocked},targetLocked:{IsTargetPositionLockedOnFocusLock},{Result != null}");
                ...
                if (IsFocusLocked && IsTargetPositionLockedOnFocusLock && Result != null)
                {
                    ...
                    Debug.Log($"lincon PreUpdateLineRenderers SetLinePoints lock");
                    SetLinePoints(Position, Result.Details.Point);
                }
                else
                {
                    Debug.Log($"lincon PreUpdateLineRenderers SetLinePoints unlock");
